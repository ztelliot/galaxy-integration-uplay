...
from stats import find_playtime, _normalize_last_played
...
def test_no_candidate():
   ...
   assert find_playtime(cards, 0, 0) == (None, None)
...
def test_playtime_one_timestamp():
   ...
   assert find_playtime(cards, 0, 0) == (300, 0)
...
def test_playtime_one_zero():
   ...
   assert find_playtime(cards, 0, 0) == (int(timeplayed), 0)
...
def test_playtime_one_timestamp_round():
   ...
   default = 0
   assert find_playtime(cards, default, default) == (timeplayed_min, default)
...
def test_playtime_good_name():
   ...
   default = 0
   assert find_playtime(cards, default, default) == (t, default)
...
def test_playtime_assasin_collection():
   ...
   default = 0
   assert find_playtime(cards, default, default) == (total_min, default)
...
def test_two_modes():
   ...
   default = 0
   assert find_playtime(cards, default, default) == (total, default)
...
def test_gametimes_two_modes_with_last_times_hours():
   """We ignore unit field and always asume seconds"""
   t1 = 2434
   t2 = 123
   total = round((t1 + t2) / 60)
   ...
   cards = [
      StatCardMock('TTPSolo', 'Play time being alone', True, t1, 'Hours', older_modify_time).as_dict(),
      StatCardMock('TTPCoop', 'Coop Play time', True, t2, 'Hours', newer_modify_time).as_dict(),
      StatCardMock('storyCompletion.gameplayed.ACR', 'ACR | Story (%)').as_dict(),
   ]
   assert find_playtime(cards) == (total, newer_modify_timestamp)


def test_default_values_when_not_played():
   """one card of type LongTimespan -> take it. null and 1970 are Uplay defaults game was not played"""
   json_stat = '''{
      "statName": "IronManDuration.Mode.IronMan",
      "displayName": "Ironman playtime",
      "locale": "en-US",
      "value": null,
      "obj": null,
      "ordinal": 1,
   ...
   default = 0
   assert find_playtime(cards, default, default) == (0, default)


def test_gametimes_champions_of_anteria():
   """We ignore unit and assume seconds"""
   json_stat = '''{
   ...
   assert find_playtime(cards) == (1189, 1492726805)
...
def test_gametimes_farcry5():
   ...
   assert find_playtime(cards) == (total_playetime_min, last_modified_timespan)
